...
export default function JoinUsPage() {
  ...
  const getLocalizedEmploymentTypeLabel = (type: string) => {
    const translationKey = `employmentTypes.${type}`;
    const translated = tJob(translationKey);
    ...
    const loadJobs = async () => {
      try {
        ...
        const jobsResponse = await strapiService.getJobs({
          pageSize: 4 // Show up to 12 jobs on join-us page
        })
                      ...
                      <CardContent className="p-8 flex flex-col flex-1">
                        <div className="mb-6 flex-1">
                          ...
                          <div className="space-y-2 text-sm text-gray-600 mb-4">
                            ...
                            <div className="flex items-center">
                              <Clock className="h-4 w-4 mr-2 text-gray-400" />
                              <span>{job?.job_detail?.employmentType ? getLocalizedEmploymentTypeLabel(job.job_detail.employmentType) : tJob('notSpecified')}</span>
                            </div>
                            <div className="flex items-center">
