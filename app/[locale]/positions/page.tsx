...
export default function AllPositionsPage() {
  ...
  // Helper function to get localized employment type label
  const getLocalizedEmploymentTypeLabel = useCallback((type: string) => {
    const translationKey = `employmentTypes.${type}`;
    const translated = t(translationKey);
    // If translation key doesn't exist, fall back to the original type
  ...
  const loadJobs = async (page: number = 1, search: string = "", experience: string = "") => {
    try {
      setLoading(true)
  ...
  }, [currentPage, searchTerm, selectedExperience])
                          ...
                          <CardContent className="p-6 flex flex-col flex-1">
                            <div className="mb-4 flex-1">
                              ...
                              <div className="space-y-2 text-sm text-gray-600 mb-4">
                                ...
                                <div className="flex items-center">
                                  <Clock className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0" />
                                  <span className="truncate">{job?.job_detail?.employmentType ? getLocalizedEmploymentTypeLabel(job.job_detail.employmentType) : t('notSpecified')}</span>
                                </div>
                                <div className="flex items-center">
